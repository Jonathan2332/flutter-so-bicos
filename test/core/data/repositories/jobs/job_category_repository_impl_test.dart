...
void main() {
  ...
  test('getAllCategories should return a Success list of JobCategory', () async {
    final Result<List<JobCategoryApiModel>> dummy = Success(jobCategoryApiModelFakeList);
    provideDummy(dummy);

    when(dataSource.getAllCategories()).thenAnswer((_) async => dummy);
    final result = await repository.getAllCategories();
    expect(result, isA<Result<List<JobCategory>>>());
    expect(result.getOrNull()?.first.name, jobCategoryApiModelFakeList.first.name);
  });

  test('getAllCategories should return a Failure with CategoryFetchAllException', () async {
    final Result<List<JobCategoryApiModel>> dummy = Failure(CategoryFetchAllException(message: ""));
    provideDummy(dummy);
